...
public Filme localizar(int codigo) {//METODO SELECT
	...
	try {
		...
		if(bd.rs.next()) {		
			c.setCodfilme(bd.rs.getInt("C�digo filme"));
			c.setNome(bd.rs.getString("nome"));
			c.setDataLancamento(bd.rs.getString("Data de lan�amento"));
			c.setDiretor(bd.rs.getString("Diretor"));
			c.setTempo(bd.rs.getString("Tempo de dura��o"));
			c.setGenero(bd.rs.getString("G�nero"));
			c.setClassificacao(bd.rs.getString("Classifica��o"));
			c.setDescricao(bd.rs.getString("Descri��o"));
			c.setCodfuncionario(bd.rs.getInt("C�digo funcion�rio"));
		}
